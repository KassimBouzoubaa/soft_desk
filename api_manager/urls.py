...
project_issues_comment_urls = [
    path(
        "project/<int:project_pk>/issues/<int:issue_pk>/comments/",
        ...
        name="project-issues-comments-list",
    ),
    path(
        "project/<int:project_pk>/issues/<int:issue_pk>/comments/<uuid:pk>/",
        CommentViewSet.as_view(
            {
                "get": "retrieve",  # Pour récupérer les détails d'une issue dans un projet
                "put": "update",  # Pour mettre à jour une issue dans un projet
...
urlpatterns = router.urls + project_issues_urls + project_issues_comment_urls
